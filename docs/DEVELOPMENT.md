...
- `npm run dev` – runs an initial build, launches the build watcher, and serves `dist/infinite-craft-helper.user.js` on `http://localhost:3000`; the served bundle advertises localhost update/download URLs so Tampermonkey tracks your live build. Add `--skip-quality` to relax watch-time lint/tests when approved.
